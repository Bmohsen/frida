...
impl fmt::Display for GeolocationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GeolocationError::LocationLookupFailed(msg) => write!(f, "Location lookup failed: {}", msg),
...
impl Geolocator {
    ...
    /// println!("Multi-source location: {}", Geolocator::format_location(&location));
    /// ```
    ...
    fn get_wifi_location(&self) -> Result<WifiLocation, GeolocationError> {
        ...
        Err(GeolocationError::WiFiError("WiFi location not implemented yet".to_string()))
    }
    ...
    fn get_system_location(&self) -> Result<String, GeolocationError> {
        ...
        // This will require platform-specific code
        Err(GeolocationError::SystemError("System location not implemented yet".to_string()))
    }
        ...
        match timeout(
            ...
            Ok(ip_result) => {
                match ip_result {
                    ...
                    Err(e) => Err(GeolocationError::IPDetectionFailed(e.to_string())),
                }
            },
    ...
    /// # Examples
    /// ```
    /// let location = geolocator.get_location_info().await?;
    /// println!("Location: {}", Geolocator::format_location(&location));
    /// ```
    ...
    pub async fn get_location_info(&self) -> Result<LocationInfo, GeolocationError> {
        // Get public IP first
        let ip = self.get_public_ip().await?;
        
        // Use iplocate to get geolocation data from the IP
        let client = iplocate::Client::new();
